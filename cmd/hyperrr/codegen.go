...
func generateResolverStruct(modules []ModuleInfo, interfaces []ResolverInterface) error {
	...
	buf.WriteString("\tProjector          *ctxengine.Projector\n")
	for _, mod := range modules {
		if mod.Name == "ctxengine" {
	...
	buf.WriteString("func NewResolver(modules []registry.Module, runner *workflow.Runner, registryStore *workflow.Registry, projector *ctxengine.Projector) *Resolver {\n")
			...
			if found {
				...
				if strings.Contains(method.Results, "error") {
					...
					if zeroValsExceptError != "" {
						buf.WriteString(fmt.Sprintf("\t\treturn %s, fmt.Errorf(\"module %s not loaded\")\n", zeroValsExceptError, targetMod.Name))
					} else {
						buf.WriteString(fmt.Sprintf("\t\treturn fmt.Errorf(\"module %s not loaded\")\n", targetMod.Name))
					}
				} else {
				...
				// Stub for unclaimed
				buf.WriteString(fmt.Sprintf("\tpanic(fmt.Errorf(\"not implemented: %s\"))\n", method.Name))
			}
