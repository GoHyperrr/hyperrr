...
func TestTUI(t *testing.T) {
	t.Run("Update and View", func(t *testing.T) {
		...
		// Update with event
		msg := WorkflowMsg{ID: "wf1", Name: "Test Workflow", State: "RUNNING", Step: "step1"}
		...
		if len(m.workflows) != 1 || m.workflows["wf1"].State != "RUNNING" {
			t.Error("workflow state not updated")
		}

		// View after update
		view = m.View()
		if !strings.Contains(view, "RUNNING") || !strings.Contains(view, "wf1") {
			t.Errorf("unexpected updated view: %s", view)
		}
		
		// Update with waiting state
		m.Update(WorkflowMsg{ID: "wf1", State: "WAITING_HUMAN"})
		view = m.View()
		if !strings.Contains(view, "WAITING_HUMAN") {
